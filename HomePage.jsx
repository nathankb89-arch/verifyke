  ...
  return (
    <div className='max-w-6xl mx-auto px-4'>
      ...
      <section className='mt-16'>
        <div className='bg-slate-900 p-8 rounded-3xl border border-slate-800'>
          <h2 className='text-3xl font-bold text-center'>What users are saying</h2>
          <div className='grid md:grid-cols-3 gap-6 mt-8'>
            <div className='bg-slate-950 p-4 rounded-2xl'>
              <p className='text-slate-200'>"Verifyke found a phishing link I almost clicked — saved me hours of trouble."</p>
              <p className='text-slate-400 mt-3 font-semibold'>— Jordan K.</p>
            </div>
            <div className='bg-slate-950 p-4 rounded-2xl'>
              <p className='text-slate-200'>"The URL scanner is fast and accurate. Love the browser extension."</p>
              <p className='text-slate-400 mt-3 font-semibold'>— Priya S.</p>
            </div>
            <div className='bg-slate-950 p-4 rounded-2xl'>
              <p className='text-slate-200'>"Clear explanations and actionable tips — perfect for non-technical users."</p>
              <p className='text-slate-400 mt-3 font-semibold'>— Marcus L.</p>
            </div>
          </div>
        </div>
      </section>

      <section className='mt-16 bg-slate-950 p-8 rounded-3xl border border-slate-800'>
        <h2 className='text-3xl font-bold'>Frequently Asked Questions</h2>
        <div className='mt-6 grid gap-4'>
          <details className='bg-slate-900 p-4 rounded-lg'>
            <summary className='cursor-pointer font-semibold'>How does Verifyke detect scams?</summary>
            <p className='text-slate-400 mt-2'>We use a combination of heuristic rules, URL reputation databases, and AI models trained on known scam content to provide risk scores and explanations.</p>
          </details>
          <details className='bg-slate-900 p-4 rounded-lg'>
            <summary className='cursor-pointer font-semibold'>Is my data private?</summary>
            <p className='text-slate-400 mt-2'>Yes. Scans are processed securely and we do not share your input with third parties. See our privacy policy for full details.</p>
          </details>
          <details className='bg-slate-900 p-4 rounded-lg'>
            <summary className='cursor-pointer font-semibold'>Can I request monitoring for a domain?</summary>
            <p className='text-slate-400 mt-2'>Yes — our live monitoring features support custom domain and keyword watches for business and personal users.</p>
          </details>
        </div>
      </section>

      <footer className='mt-12 py-8 text-center text-slate-400'>
        © {new Date().getFullYear()} Verifyke — Built for safer browsing
      </footer>
    </div>
  )
}
